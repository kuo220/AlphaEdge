...
class ShioajiAccount:
    ...
    # API logout
    @staticmethod
    def API_logout(api: sj.Shioaji) -> None:
        """ API 登出 """
    ...
    def get_settlement_capital(api: sj.Shioaji) -> float:
        ...
        settlements: pd.DataFrame = pd.DataFrame([s.__dict__ for s in api.settlements(api.stock_account)])
    ...
    def get_invest_capital(api: sj.Shioaji, capital: float) -> float:
        ...
        total_capital: float = ShioajiAccount.get_settlement_capital(api) # 帳戶可用的總資金
        settlements: pd.DataFrame = pd.DataFrame([s.__dict__ for s in api.settlements(api.stock_account)]) 
        capital_quota: float = capital + settlements.loc[2, 'amount'] # 目前剩餘的投資額度
        invest_capital: float = total_capital if total_capital <= capital_quota else capital_quota
    ...
    def get_realized_pnl(api: sj.Shioaji) -> int:
        ...
        pnl_df: pd.DataFrame = pd.DataFrame(s.__dict__ for s in api.list_profit_loss(api.stock_account))
    ...
    def get_unrealized_pnl(api: sj.Shioaji) -> int:
        ...
        positions_df: pd.DataFrame = pd.DataFrame(s.__dict__ for s in api.list_positions(api.stock_account))
        unrealized_pnl: int = round(positions_df.pnl.sum()) if len(positions_df) > 0 else 0
        return unrealized_pnl
    
    
    @staticmethod
    def generate_trade_record(api: sj.Shioaji, trade_record_path: str) -> None:
        """ 製作投資紀錄表（模擬模式） """
        
        columns_name: List[str] = ["date", "stock_id", "order_action", "seqno", "volume", "buy_price", "sell_price", "pnl"]
        trade_record_df: pd.DataFrame = pd.DataFrame(columns=columns_name)

        api.update_status(api.stock_account)
        trade_list: List[Trade] = api.list_trades()
        pnl_list: List[Union[StockProfitLoss, FutureProfitLoss]] = api.list_profit_loss()
        ...
        for trade in trade_list:
            if trade.status.status == Status.Filled:
                ...
                ave_deal_price: float = round(
                    sum(deal.price * deal.quantity for deal in trade.status.deals) / trade.order.quantity, 
                    2
                )

                trade_record: pd.DataFrame = pd.DataFrame(
                    [[
                        trade.status.order_datetime, 
                        trade.contract.code,
                        trade.order.action, 
                        trade.order.seqno, 
        ...
        # open existed record
        exist_trade_record: pd.DataFrame = pd.read_csv(trade_record_path) if os.path.exists(trade_record_path) else pd.DataFrame(columns=columns_name)   
