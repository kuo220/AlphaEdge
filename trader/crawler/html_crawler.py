...
class CrawlHTML:
    ...
    def crawl_twse_stock_info() -> pd.DataFrame:
        ...
        response: requests.Response = requests.get(URLManager.get_code_url("TWSE_CODE_URL"))
    ...
    def crawl_tpex_stock_info() -> pd.DataFrame:
        ...
        response: requests.Response = requests.get(URLManager.get_code_url("TPEX_CODE_URL"))
    ...
    def crawl_stock_list() -> List[str]:
        ...
        # 取得上市公司代號
        twse_df: pd.DataFrame = CrawlHTML.crawl_twse_stock_info()
        twse_stock_list: List[str] = twse_df['證券代號'].to_list()
        print(f"* TWSE stocks: {len(twse_stock_list)}")

        # 取得上櫃公司代號
        tpex_df: pd.DataFrame = CrawlHTML.crawl_tpex_stock_info()
