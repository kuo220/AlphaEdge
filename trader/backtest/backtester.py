...
class Backtester:
    ...
    def place_open_order(self, stock: StockOrder) -> Optional[StockTradeRecord]:
        ...
        open_cost: float = StockUtils.calculate_transaction_commission(buy_price=stock.price, volume=stock.volume)
    ...
    def place_close_order(self, stock: StockOrder) -> Optional[StockTradeRecord]:
        ...
        position_value: float = stock.price * stock.volume * Units.LOT
        close_cost: float = StockUtils.calculate_transaction_commission(sell_price=stock.price, volume=stock.volume)
        ...
        if position is not None and not position.is_closed:
            ...
            if position.position_type == PositionType.LONG:
                ...
                position.tax = StockUtils.calculate_transaction_tax(stock.price, stock.volume)
                position.transaction_cost = position.commission + position.tax
                position.realized_pnl = StockUtils.calculate_net_profit(position.buy_price, position.sell_price, position.volume)
                position.roi = StockUtils.calculate_roi(position.buy_price, position.sell_price, position.volume)
