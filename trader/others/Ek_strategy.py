...
class EnergyStrategy(Strategy):
    ...
    def backtest(self) -> None:
        ...
        # Print Result
        print(
            f"* Total Profit: {self.transaction_record['Profit'].sum()}, "
    ...
    def run(self) -> pd.DataFrame:
        ...
        def check_data_exist_next_day(stock_list: pd.DataFrame) -> pd.DataFrame:
            ...
            for stock in stock_list.columns:
                if (stock in next_price.columns) and (
                    not next_price[stock].isna().values
                ):
                    valid_stock_list.append(stock)
                else:
        ...
        filtered_columns = [
            ...
            if col.isdigit() and len(col) == 4 and 1101 <= int(col) <= 9958
        ]
        ...
        today_increase = (
            (close_price - close_price.shift(periods=1))
            / close_price.shift(periods=1)
            * 100
        ).iloc[-1:]
        today_increase_nine_up = today_increase.iloc[-1] > 9
        today_increase_nine_up_stocks = today_increase.loc[
            :, today_increase_nine_up
        ].columns
        ...
        yesterday_increase_less_nine_stocks = yesterday_increase.loc[
            :, yesterday_increase_less_nine
        ].columns

        # Condition 1 & Condition 2
        selected_stocks = set(today_increase_nine_up_stocks) & set(
            yesterday_increase_less_nine_stocks
        )
        ...
        selected_stocks = selected_stocks.iloc[-1:][valid_stock_list].loc[
            :, volume_5000_up
        ]
        ...
        if len(self.stocks_inventory) == 0:
            if len(selected_stocks.columns) < self.buy_stock_num:
                ...
                return close_price[selected_stocks.columns].iloc[
                    -1:, : self.buy_stock_num
                ]
        else:
            if len(selected_stocks.columns) < self.buy_stock_num - len(
                self.stocks_inventory
            ):
                return close_price[selected_stocks.columns].iloc[-1:]
            else:
                return close_price[selected_stocks.columns].iloc[
                    -1:, : (self.buy_stock_num - len(self.stocks_inventory))
                ]
    ...
    def produce_portfolio(
        ...
        threshold_with_minimum_fee_rate = (lowest_fee - add_cost) / (
            0.001425 * discount
        )
        ...
        invest_amount_per_stock = total_invest_money / len(stock_list)
        stock_cannot_buy_1000_shares = (
            np.floor(invest_amount_per_stock / (stock_list * 1000)) == 0
        ).any()
        ...
        for stock_id, shares in selected_stock.items():
            print(
                f"{stock_id:<9} {shares:<7} {round(close_price.iloc[-1].loc[stock_id] * shares * 1000):<6}"
            )
        print()
    ...
    def sell_stocks(self) -> None:
        ...
        def calculate_friction_cost(
            ...
            security_exchange_tax_rate=0.003,
            fee_discount=0.3,
        ):
            """
            ...
            """
            buy_fee = (
                buy_price * stock_share * 1000 * transaction_fee_rate * fee_discount
            )
            sell_fee = (
                sell_price
                * stock_share
                * 1000
                * (transaction_fee_rate * fee_discount + security_exchange_tax_rate)
            )
        ...
        for stock in self.stocks_inventory:
            if self.sell_time == "open":
                ...
                increase_ratio = round(
                    (open_price.iloc[-1] - close_price.iloc[-2])
                    / close_price.iloc[-2]
                    * 100,
                    2,
                )
            ...
            # TODO: limit up
                ...
                open_increase_ratio = round(
                    (open_price / close_price.iloc[-1:] - 1) * 100, 2
                ).iloc[-1]
                close_increase_ratio = round(
                    (close_price / close_price.shift(periods=1) - 1) * 100, 2
                ).iloc[-1]

                if (
                    open_increase_ratio[stock.stock_id] >= 9
                    and close_increase_ratio[stock.stock_id] >= 9.5
                ):
                    print(f"- Stock ID: {stock.stock_id}")
                    print(
                        f"  - Limit up stock: {stock.stock_id}, Increase ratio: {close_increase_ratio[stock.stock_id]}"
                    )
                    print("  - Status: Hold")
                    continue
                sell_price = open_price.iloc[-1][stock.stock_id]
            ...
            buy_fee, sell_fee = calculate_friction_cost(
                stock.buy_price, sell_price, stock.stock_share
            )
            total_cost = buy_fee + sell_fee
            total_profit = round(
                (sell_price - stock.buy_price) * stock.stock_share * 1000 - total_cost
            )
            roi = round(
                total_profit / (stock.buy_price * stock.stock_share * 1000) * 100, 2
            )
            ...
            record = pd.Series(
                [
                    ...
                    stock.profit,
                    stock.ROI,
                    self.accumulated_capital,
                ],
                index=record_col_name,
            )
            self.transaction_record = pd.concat(
                [self.transaction_record, record.to_frame().T], ignore_index=True
            )

            # print sell result
            print(f"- Stock ID: {stock.stock_id}")
            print(
                f"  - Buy Price: {stock.buy_price}, Sell Price: {stock.sell_price}, Profit: {stock.profit}, Profit(%): {stock.ROI}"
            )

            # Warning message for selling too many stocks
            if stock.stock_share > stock.volume * self.volume_limit:
                print(
                    f"  - Volume Limit: {round(stock.volume * self.volume_limit)}, Sell Volume: {stock.stock_share}, Ratio: {round(stock.stock_share * 100 / stock.volume, 2)}%\n"
                )

            print(
                f"  - Before Sell Accumulated Capital: {self.accumulated_capital - total_profit}"
            )
        ...
        print(
            f"- Today Total Profit: {self.accumulated_capital - original_accumulated_capital}"
        )
    ...
    def summarize_transaction_stats(self) -> None:
        ...
        # set figure layout
        def set_fig_layout(
            fig, title: str = "", xaxis_title: str = "", yaxis_title: str = ""
        ) -> None:
            fig.update_layout(
        ...
        file_path = os.path.join(
            ...
            f"{self.start_date.strftime('%Y%m%d')}_{self.end_date.strftime('%Y%m%d')}_{self.params['capital']}_{self.buy_stock_num}_stocks",
        )
        ...
        transaction_record_path = os.path.join(
            file_path, f"transaction_record_{self.buy_stock_num}.csv"
        )
        self.transaction_record.to_csv(transaction_record_path)

        # Plot the transaction statistics
        # Fig1: Accumulated capital
        accumulated_capital_fig = px.line(
            self.transaction_record, x="Sell Date", y="Accumulated Capital"
        )
        accumulated_capital_path = os.path.join(
            file_path, f"accumulated_capital_{self.buy_stock_num}.png"
        )

        total_profit = self.transaction_record["Profit"].sum()
        total_roi = round(
            self.transaction_record["Profit"].sum() / self.initial_capital * 100, 2
        )
        win_ratio = round(
            len(
                self.transaction_record[self.transaction_record["Profit"] > 0]["Profit"]
            )
            / len(self.transaction_record["Profit"])
            * 100
        )
        invest_info = (
        ...
        set_fig_layout(
            fig=accumulated_capital_fig,
            title=f"Return (Initial Capital: {self.initial_capital}, {self.start_date.strftime('%Y/%m/%d')} ~ {self.end_date.strftime('%Y/%m/%d')})",
            xaxis_title="Date",
            yaxis_title="Accumulated Capital",
        )
        ...
        everyday_profit_path = os.path.join(
            file_path, f"everyday_profit_{self.buy_stock_num}.png"
        )
        set_fig_layout(
            fig=everyday_profit_fig,
            title=f"Everyday Every Stock's Profit ({self.start_date.strftime('%Y/%m/%d')} ~ {self.end_date.strftime('%Y/%m/%d')})",
            xaxis_title="Date",
            yaxis_title="Profit",
        )
        ...
        close_price = self.data.get(
            "price", "收盤價", (self.end_date - self.start_date).days
        )
        self.data.date = current_date

        benchmark_price = close_price[self.params["benchmark_id"]][
        ...
        accumulated_capital = (
            self.transaction_record["Accumulated Capital"]
            .groupby(self.transaction_record["Accumulated Capital"].index)
            .last()
            .astype(float)
        )
        ...
        benchmark_buy_shares = np.floor(
            self.initial_capital / (benchmark_buy_price * 1000)
        )

        if benchmark_buy_shares > 0:
            benchmark_net_worth = (
                benchmark_price / benchmark_price.iloc[0] * self.initial_capital
            )
            ...
            net_worth_fig_path = os.path.join(
                file_path, f"net_worth_{self.buy_stock_num}.png"
            )

            benchmark_roi = round(
                (benchmark_price.iloc[-1] / benchmark_price.iloc[0] - 1) * 100, 2
            )
            compare_info = (
            ...
            set_fig_layout(
                fig=net_worth_fig,
                title=f"Energy Strategy & {self.params['benchmark_id']} Net Worth ({self.start_date.strftime('%Y/%m/%d')} ~ {self.end_date.strftime('%Y/%m/%d')})",
                xaxis_title="Date",
