...
class QuantXCrawler:
    ...
    def __init__(self):
        self.table_without_stockid: List[str] = [
            "tw_total_pmi",
            "tw_total_nmi",
    ...
    def crawl_benchmark_return(self, date) -> pd.DataFrame:
        ...
        df["發行量加權股價報酬指數"] = df["發行量加權股價報酬指數"].apply(
            lambda x: round(x, 2)
        )
    ...
    def crawl_margin_transactions(self, date) -> pd.DataFrame:
        ...
        url_names: List[str] = [
            "TWSE_MARGIN_FUND_URL",
            "TWSE_MARGIN_ETF_URL",
            "TWSE_MARGIN_TDR_URL",
            "TWSE_MARGIN_STOCK_URL",
        ]
        url_list: List[str] = [
            URLManager.get_url(url_name, date=date_str) for url_name in url_names
        ]
        c = 0
        df = None
        for url in url_list:
    ...
    def crawl_tpex_price_old_1(self, date) -> Optional[pd.DataFrame]:
        ...
        for row in table.find_all("tr"):
            ...
            if len(cells) > 17:  # 根據你提供的數據結構，確保列數正確
                ...
                data = {
                    columns[idx]: cells[i].text.strip() for idx, i in enumerate(index)
                }
        ...
        df.drop(df.columns[[6, 12, 13, 14, 15]], axis=1, inplace=True)
    ...
    def crawl_tpex_price_old_2(self, date) -> Optional[pd.DataFrame]:
        ...
        datestr = (
            str(int(datestr[0:4]) - 1911) + "/" + datestr[2:4] + "/" + datestr[4:6]
        )
    ...
    def crawl_old_price(self, date) -> Optional[pd.DataFrame]:
        ...
        try:
            r = self.requests_post(
                URLManager.get_url("TWSE_CLOSING_QUOTE_URL", date=date_str)
            )
    ...
    def crawl_tpex_monthly_report(self, date):
        ...
        next_month = datetime.date(
            date.year + int(date.month / 12), ((date.month % 12) + 1), 10
        )
    ...
    def crawl_finance_statement2019(self, year, season):
        def ifrs_url(year, season):
            url: str = URLManager.get_url("IFRS_URL", year=year, season=season)
            print(url)
            return url
    ...
    def crawl_finance_statement(self, year, season, stock_ids):
        ...
        def download_html(year, season, stock_ids, report_type="C"):
            headers = {
                ...
                "User-Agent": DataUtils.generate_random_header()["User-Agent"],
            }
            pbar = tqdm(stock_ids)
            for i in pbar:
                ...
                pbar.set_description(
                    "parse htmls %d season %d stock %s" % (year, season, str(i))
                )
    ...
    def add_to_sql(self, conn, name, df):
        ...
        ret = (
            pd.read_sql("select * from " + name, conn, index_col=["stock_id", "date"])
            if exist
            else pd.DataFrame()
        )
        ...
        try:
            ...
            ret = pd.read_csv(
                "backup.csv", parse_dates=["date"], dtype={"stock_id": str}
            )
            ret["stock_id"] = ret["stock_id"].astype(str)
    ...
    def add_to_sql_without_stock_id_index(self, conn, name, df):
        exist = self.table_exist(conn, name)
        ret = (
            pd.read_sql("select * from " + name, conn, index_col=["date"])
            if exist
            else pd.DataFrame()
        )
    ...
    def widget(self, conn, table_name, crawl_func, range_date):
        ...
        if self.table_exist(conn, table_name):
            label = widgets.Label(
                table_name
                + " (from "
                + self.table_earliest_date(conn, table_name).strftime("%Y-%m-%d")
                + " to "
                + self.table_latest_date(conn, table_name).strftime("%Y-%m-%d")
                + ")"
            )
        else:
            label = widgets.Label(
                table_name + " (No table found)(對於finance_statement是正常情況)"
            )
...
class FinanceDataHandler:
    ...
    def patch2019(self, df):
        ...
        df.columns = pd.MultiIndex(
            levels=[df.columns.levels[1], df.columns.levels[0]],
            codes=[df.columns.codes[1], df.columns.codes[0]],
        )
    ...
    def read_html2019(self, file):
        ...
        return [
            pd.DataFrame(),
            self.patch2019(dfs[0]),
            self.patch2019(dfs[1]),
            self.patch2019(dfs[2]),
        ]
    ...
    def pack_htmls(self, year, season, directory):
        ...
        for i in pbar:
            ...
            stock_id = i.split(".")[0]
            pbar.set_description(
                "parse htmls %d season %d stock %s" % (year, season, stock_id)
            )
        ...
        # 做清理
        ret = {
            "balance_sheet": self.clean(year, season, balance_sheet),
            "income_sheet": self.clean(year, season, income_sheet),
            "income_sheet_cumulate": self.clean(year, season, income_sheet_cumulate),
            "cash_flows": self.clean(year, season, cash_flows),
        }
        ...
        ret["income_sheet_cumulate"].columns = (
            "累計" + ret["income_sheet_cumulate"].columns
        )

        pickle.dump(
            ret,
            open(
                "data/financial_statement/pack" + str(year) + str(season) + ".pickle",
                "wb",
            ),
        )
    ...
    def fill_season4(self, tbs):
        ...
        for y in years:
            ...
            df4 = (
                tbs["income_sheet_cumulate"][ds4 & ys]
            ...
            yps = (
                tbs["income_sheet_cumulate"].reset_index("stock_id").index.year == y - 1
            )
            ds3 = tbs["income_sheet_cumulate"].reset_index("stock_id").index.month == 11
            df3 = (
                tbs["income_sheet_cumulate"][ds3 & yps]
                .apply(lambda s: pd.to_numeric(s, errors="coerce"))
                .reset_index("date")
            )
            ...
            diff = (
                diff[list(c1) + ["date"]].reset_index().set_index(["stock_id", "date"])
            )
    ...
    def html2db(self, date):
        ...
        self.pack_htmls(
            year,
            season,
            os.path.join("data", "financial_statement", str(year) + str(season)),
        )
