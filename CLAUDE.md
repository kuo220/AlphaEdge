...
3. **進度追蹤表**：全份工作的單一進度來源，欄位固定如下：
   ...
   - 編號用 `S1`、`S2`…；有分階段時用 `P{階段}-{序號}`（例如 `P2-3`）。
...
1. **新增** `.md` 時：在 `index.md` 表格新增一列（優先級、狀態、說明、進度、相依），並在 [`backlog/README.md`](backlog/README.md) 的項目清單新增一列。
2. **實作推進**時（子步驟完成、狀態變更）：同步更新該列的「狀態」與「進度」，不可只改單一文件內的進度表。
3. **完成整份移出** `backlog/` 時：同步刪除 `index.md` 與 `README.md` 中對應那一列。
