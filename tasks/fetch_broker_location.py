...
def fetch_broker_location() -> pd.DataFrame:
    """Fetch broker info from FinMind API as a DataFrame"""
    load_dotenv()
    token: str | None = os.environ.get("API_TOKEN")
    if not token:
    ...
    data: pd.DataFrame = pd.DataFrame(payload["data"])
    print(data.head(10))
    return data
...
if __name__ == "__main__":
    df: pd.DataFrame = fetch_broker_location()
    csv_path: Path = save_to_csv(df)
