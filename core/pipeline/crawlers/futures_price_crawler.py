...
class FuturesPriceCrawler(BaseDataCrawler):
    ...
    def validate_product(product: str) -> None:
        """
        商品代碼防呆：`FUTURES_TARGET_PRODUCTS` 是手寫字面值，拼錯不會有編譯期提示

        一次回補動輒上萬次請求，錯誤代碼會安靜地全部查無資料而看似「整段都是假日」，
        因此在送出請求前就擋下來
        """

        valid: set = {member.value for member in FuturesProduct}

        if product not in valid:
            raise ValueError(
                f"未知的期貨商品代碼：{product}。"
                f"可用代碼請見 core/utils/constant.py 的 FuturesProduct：{sorted(valid)}"
            )
