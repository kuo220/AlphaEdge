...
1. **新增** `.md` 時：在 `index.md` 表格新增一列（優先級、狀態、說明、進度、相依）。檔案清單**只**維護在 `index.md`，不要在其他文件另立一份清單。
...
`backlog/` 只放「**尚未實作**」的規劃，項目一旦實作完成即**整份移出**。

`docs/` 只放**現行的使用與架構說明**（README〈文件〉索引列得到的那些），**不放過程紀錄**：

1. 完成的工作改變了現行設計或用法 → 把「**現在怎麼運作、為什麼這樣設計、有什麼已知限制**」
   寫進 `docs/` 對應的正式文件（沒有就新增，並加進 README 兩份語言版的文件索引）。
   **不要把整份 backlog 搬進 `docs/`**：進度表、完成紀錄、驗收數字、事故經過、日期與 commit
   一律不帶過去——那些留在 git 歷史。
2. 只是實作過程的規劃筆記 → 直接刪除。
3. 方案已選定但尚未實作 → 留在 `backlog/` 當待辦，不放 `docs/`。
4. 以上情況都要同步刪掉或更新 `index.md` 對應那一列（見第 4 節第 3 點）。
5. 程式碼註解**不引用** `docs/`／`backlog/` 的章節、步驟編號或健檢發現編號——理由直接寫在註解裡；
   文件可以指向程式碼，程式碼不指向文件。
