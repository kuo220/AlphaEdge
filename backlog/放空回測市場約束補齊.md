...
|------|----------|----------|----------|:----:|--------------|
| S1 | 融券餘額／信用交易 ETL（crawler→cleaner→loader→updater→API） | `core/pipeline/*/stock_margin_*.py`、`core/api/stock_margin_api.py` | `tasks/update_db.py --target margin` 可日更；抽樣比對證交所原始數據 | ✅ | CLI 參數定為 `--target margin`（偏離原規格的 `stock_margin`，見 S1）；歷史回補的執行追蹤改由 [爬蟲入庫時序與中斷風險改善.md](爬蟲入庫時序與中斷風險改善.md) S1 承接 |
...
- **margin 歷史回補的執行追蹤**（2026-08-15）：改由 [爬蟲入庫時序與中斷風險改善.md](爬蟲入庫時序與中斷風險改善.md) S1 承接——實查發現回補程序中斷後 CSV（2013-01-02 ～ 2020-10-13）未入庫、表仍 0 列，在回補完成前 `check_borrowable=True` 實際上不會生效（查無資料一律放行）。
