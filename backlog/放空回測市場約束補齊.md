...
- **做法**：
  ...
  - ⚠️ **`check_borrowable` 目前是死碼**（2026-08-07 實查）：欄位定義在 `cost_model.py:40`，但**全專案沒有任何呼叫端**，使用者設成 `True` 不會生效也不會報錯。本步驟不只是「接上資料」，而是要讓這個開關第一次真的有作用；同批的 `allow_below_reference`、`day_trade_whitelist` 也是死碼，其防呆處理見 `回測引擎執行真實度補強`（2026-08-15 八步全數完成並移出 `backlog/`） S1。
...
- **進度**：S1 ✅（2026-08-01）；**S2 ✅**（2026-08-15，券源檢核接進 `FillModel`）；S3 🔄（除權息 ETL 已完成，回測層的停券回補與股利補償未動）；S4~S6 ⏸
