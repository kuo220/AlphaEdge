...
| S3 | 除權息行事曆 ETL ＋ 停券日強制回補與股利補償 | `core/pipeline/*/stock_dividend_*.py`、`core/backtest/backtester.py` | 停券日觸發強制回補；除息日扣股利補償 | 🔄 | **ETL 部分已於 2026-08-15 由 `股價還原與除權息調整`（2026-08-15 完成並移出 `backlog/`，見 [`docs/exchanges/data_coverage.md`](../docs/exchanges/data_coverage.md)） S1 完成**（共用資料源，只做一次），`StockDividendAPI.get_cash_dividend_map()` 可直接取用。**上櫃的現金股利由櫃買中心直接提供、無缺值；僅上市權息並存的標的為 `NULL`**（2024 年 1184 筆中 85 筆，理由見該文件〈已知限制〉），股利補償動工前須另接股利政策資料源或明確拒絕處理該類標的。回測層的停券回補與股利補償尚未實作 |
...
| 停券日資料源（除權息／股東會） | ⚠️ 除權息部分已補齊（2026-08-15，`dividend` 表） | 引擎已有 `force_cover_dates` 掛點與 `forced_cover_suspended` 事件；缺的是「從 `dividend` 表自動填停券日」與股東會停券資料 | S3（第二順位） |
| 股利補償現金流 | ⚠️ 資料部分可得 | 上櫃現金股利完整；上市權息並存標的為 `NULL`（見 S3），須另接資料源或明確拒絕處理 | S3（附帶） |
...
- **與價格還原的關係（2026-08-07）**：本步驟的除權息 ETL 與 `股價還原與除權息調整`（2026-08-15 完成並移出 `backlog/`，見 [`docs/exchanges/data_coverage.md`](../docs/exchanges/data_coverage.md)） **共用同一份資料源**（除權息交易日、除息金額、除權比率）。兩者建議合併排程：先由本步驟或該文件其一完成 ETL，另一方直接取用。注意兩者解的是不同問題——本步驟處理「停券回補與股利補償現金流」，該文件處理「價格序列本身的跳空」，**都做才不會重複計算或互相抵銷**，實作時須確認：若價格已還原，則除息日的帳面損益已不含跳空，股利補償只需處理放空者付給出借方的那一筆。
...
- **進度**：S1 ✅（2026-08-01）；**S2 ✅**（2026-08-15，券源檢核接進 `FillModel`）；S3 🔄（除權息 ETL 已完成，回測層的停券回補與股利補償未動）；S4~S7 ⏸
