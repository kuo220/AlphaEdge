...
- **背景／問題**：專案目前以 `core/database/tw_stock.db`（SQLite3）為主要儲存，且是「直接耦合」——多處 `import sqlite3` 與 `sqlite3.connect(...)`、SQLite 專屬檢查（`sqlite_master`、`PRAGMA table_info`）、Loader／Updater／API 直接持有 `sqlite3.Connection`、測試大量依賴本地 SQLite 檔案。這不是改連線字串就能解決的問題。
...
| Phase1-2 | `TW_STOCK_DB_PATH` 直連改為經由 engine（含 SQLite fallback） | `core/config.py`、各 API | 不改業務邏輯前提下 API 可讀到資料 | ⬜ | 相依 Phase1-1；優先讀 `DATABASE_URL`，未設定則 fallback SQLite |
...
| Phase4-1 | 測試 fixture 改為 PostgreSQL 測試資料庫 | `tests/conftest.py` | 不再直接 patch 檔案型 DB 路徑 | ⬜ | 相依 P2-* |
...
| Phase5-3 | 更新 README 與部署文件 | `README.md`、`README_zh.md`、部署文件 | 團隊可依文件重現部署 | ⬜ | 相依 Phase5-2 |

---

## 遷移原則

- 將目前以 `core/database/tw_stock.db` 為主的 SQLite 存取，改為 PostgreSQL。
- 先確保「功能等價」再做「效能優化」。
...
- **做法**：二選一——
  - **方案 A：pgloader（推薦先嘗試）**。優點是快速、表結構與資料可一次搬運；缺點是轉型規則需驗證，**中文欄位名稱需特別檢查**。
    ...
    ```bash
    pgloader sqlite:///absolute/path/to/core/database/tw_stock.db postgresql://postgres:postgres@localhost:5432/alphaedge
    ```
...
### Phase4-1. 測試 fixture 改為 PostgreSQL 測試資料庫 ⬜
...
- **相關程式**：`core/pipeline/utils/sqlite_utils.py`、`core/pipeline/tw/loaders/*`、`core/pipeline/tw/updaters/*`、`core/api/*`、`core/config.py`、`tasks/*`、`tests/`
...
- **命名軸線收斂（2026-09-01 完成）交接過來的兩項**，皆刻意留到本計畫的遷移批次一起做，
  ...
  2. **`core/pipeline/downloads/` 的目錄形狀**：現為 `tw_stock/`／`tw_futures/`（市場 ＋ 商品
