...
- **缺口怎麼發現的**：2026-09-04 收斂回測端的健檢 F-058（期貨同契約多空各佔
  保證金）時反推出來的。交易所對**同商品跨月份**的價差部位（calendar spread）
  只收單邊或按專屬費率收取，而本專案入庫的只有原始／維持兩種
  （`futures_margin_history` 每口金額、`stock_futures_margin_rate_history` 適用比例），
  **第三張費率表沒有來源**。回測端因此讓跨月份兩腿各繳全額。
- **暫緩原因**：不是做不到，而是**現在沒有需求**。目前唯一的期貨策略
...
- **啟動時的做法**：先確認 TAIFEX 公告裡價差部位保證金的**收取語意**
  （單邊全額 vs 專屬金額 vs 比例），再決定是併入 `futures_margin_history`
  加一個型別欄，還是另立一張表——**不要在確認語意之前先建表**，S3 的分表依據
  就是這樣被修正過一次（原本寫「指數類 vs 股票類」，實查才發現是「金額 vs 比例」）。
- **相依**：S4（歷史序列已落地）；不擋任何現有工作。
