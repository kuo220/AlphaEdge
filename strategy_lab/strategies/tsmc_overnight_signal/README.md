...
| **交易成本** | **主結果**走 `StockUtils.calculate_transaction_commission()`：手續費 0.1425% × 券商折扣 0.3 ＝ **0.04275%**，且**單筆最低 20 元**；賣出另加證交稅 **0.3%**。另有一條向量化診斷路徑用**未折扣**的 0.1425%／0.4425%（見 §4.1 的說明）。 |
...
- **交易成本扣法**：本研究有**兩條回測路徑，費率不同**，讀數字前要先確認在看哪一條。

  | 路徑 | 函式 | 手續費 | 證交稅 | 最低手續費 | 用途 |
  |------|------|--------|--------|-----------|------|
  | **主結果** | `run_backtest_with_signal()` | 0.1425% × 折扣 0.3 ＝ **0.04275%** | 0.3%（賣出） | **20 元** | `metrics_summary.csv`、報告中引用的所有數字 |
  | 診斷 | `run_vectorized_continuous_backtest()` | **0.1425%**（未折扣） | 0.3%（賣出） | 無 | 只用來對照「整張限制與最低手續費造成多少落差」 |

  **主結果與 `core/` 的 `CostConfig` 同源**（同樣走 `StockUtils`），所以研究結論搬進生產不會因為費率而改變；診斷路徑刻意保留未折扣費率，是為了看出「整張買賣 ＋ 最低手續費」本身的影響有多大。

  - 部位由 0 變 1（買進）時扣買進手續費；由 1 變 0（賣出）時扣賣出手續費 ＋ 證交稅。
