...
def test_merge_logic():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cleaned_cols_file = (
            test_meta_dir / "monthly_revenue_report_cleaned_columns.json"
        )
        ...
        try:
            ...
            loader.monthly_revenue_report_cleaned_cols_path = (
                test_meta_dir / "monthly_revenue_report_cleaned_columns.json"
            )
            ...
            # 暫時移除第二個 CSV 檔案
            csv_file_2_temp = (
                test_downloads_dir / "monthly_revenue_report_2025_7_batch2.csv.tmp"
            )
            ...
            # 檢查具體記錄
            cursor.execute(
                f"SELECT year, month, stock_id, 公司名稱 FROM {MONTHLY_REVENUE_TABLE_NAME}"
            )
            ...
            assert (
                count_after_second == 4
            ), f"預期 4 筆記錄（3 + 1 新記錄），實際 {count_after_second} 筆"

            # 檢查具體記錄
            cursor.execute(
                f"SELECT year, month, stock_id, 公司名稱 FROM {MONTHLY_REVENUE_TABLE_NAME} ORDER BY stock_id"
            )
            records = cursor.fetchall()
            logger.info(f"資料庫中的記錄：{records}")
            expected_records = [
            ...
            # 檢查是否有重複的 1101
            cursor.execute(
                f"SELECT COUNT(*) FROM {MONTHLY_REVENUE_TABLE_NAME} WHERE stock_id = '1101'"
            )
            ...
            assert (
                count_after_third == 4
            ), f"預期仍為 4 筆記錄，實際 {count_after_third} 筆"
            ...
            config_module.MONTHLY_REVENUE_REPORT_DOWNLOADS_PATH = (
                original_downloads_path
            )
