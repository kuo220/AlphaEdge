...
| --- | --- | --- | --- |
| `StockPriceAPI` | `core/api/stock_price_api.py` | SQLite | `price` |
| `StockChipAPI` | `core/api/stock_chip_api.py` | SQLite | `chip` |
| `MonthlyRevenueReportAPI` | `core/api/monthly_revenue_report_api.py` | SQLite | `monthly_revenue` |
| `FinancialStatementAPI` | `core/api/financial_statement_api.py` | SQLite | 財報相關表 |
| `FinMindAPI` | `core/api/finmind_api.py` | SQLite | `taiwan_stock_info*`, `taiwan_securities_trader_info`, `taiwan_stock_trading_daily_report_secid_agg` |
| `StockTickAPI` | `core/api/stock_tick_api.py` | DolphinDB | `tickDB` / `tick` |
...
- 文件中的資料範圍以 `core/config.py` 預設日期與實際本地資料庫內容為準。
