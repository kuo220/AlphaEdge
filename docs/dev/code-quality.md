...
| F-010 | `core/pipeline/tw/crawlers/stock_price_crawler.py` | `crawl()` 把 `crawl_twse_price()`／`crawl_tpex_price()` 的回傳值指派後未使用（F841 ×2）；確認是否為未完成的合併邏輯。**2026-09-09 結論：不做，F841 已不存在**。現行 `crawl()` 只呼叫不指派，`ruff check --select F841` 通過。它不是未完成的合併邏輯，而是 `BaseCrawler.crawl()` 這個抽象方法的實作，`stock_chip_crawler`／`stock_margin_crawler` 三兄弟寫法逐字相同；真正的工作走 `crawl_twse_*`／`crawl_tpex_*`，`CrawlResult` 由 updater 直接接（`stock_price_updater.py:125-126`），沒有資訊在此遺失 |
| F-011 | `core/utils/callback.py` | `OrderState` 先從 `shioaji.constant` import 再被 `.constant.OrderState` 覆蓋（F811）。**2026-09-09 已修**：刪掉被覆蓋的那一行 import（行為零改變，本來就是後者生效）。**當時能運作是靠巧合**——`order_cb` 收到的 `stat` 是 Shioaji 那份，比較的是專案自己那份，兩者都是 `str` Enum 才比得起來；值一旦漂開不會拋錯，只是實盤成交回報安靜地不發通知。新增 `tests/test_order_state_parity.py` 兩條盯住兩份的成員與值一致，並在 `order_cb` 上方註明這件事 |
| F-023 | `core/adapters/tw/stock_quote_adapter.py` | `filtered_stock_ids` 用 list 做 `in` 判斷，全市場 2,000 檔 × 每日；改 set。**2026-09-09 複查：已修，無事可做**。現行是 `filtered_stock_ids: Set[str] = set(...)`，原處已留註解說明「這是每根 bar 都會跑的熱路徑，對 list 做 `in` 是線性掃描」 |
| F-029 | `core/api/tw/finmind_api.py`、`stock_tick_api.py` | 不接受共用連線注入、各自開連線；前者覆蓋率 0%。**2026-09-09 `finmind_api.py` 已修**：`__init__(conn=None)` ＋ `owns_conn`，與其他 `core/api/tw/` 同慣例；沒有 `owns_conn` 時 `BaseDataAPI.close()` 會**預設為 True 而關掉別人的共用連線**，其他持有者接著拿到已關閉的連線。新增 `tests/test_finmind_api.py` 11 條離線測試（in-memory SQLite），該檔由**全 repo 唯一 0% 覆蓋的 `core/api` 檔**變為有覆蓋——原有的 `tests/manual_finmind_api.py` 需要真的 `tw_stock.db`，pytest 不會收集它。**`stock_tick_api.py` 不動**：它連的是 DolphinDB 不是 SQLite，共用 SQLite 連線對它沒有意義 |
...
| F-088 | `strategy_lab/` | 13 個 `output/*.html` 在 `.gitignore` 規則加入前被追蹤——**2026-09-04 已 `git rm --cached` 移出版控**（檔案留在本機，`git ls-files strategy_lab \| grep output/` 為空）。**後半仍未處理**：`tech_new_high` 的 `END_DATE` 寫死、存活者偏差未列入限制、`load_price_panel()` 繞過 API 直接下 SQL |
