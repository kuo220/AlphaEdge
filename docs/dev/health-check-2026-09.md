...
>   | 等級 | 條數 | 去處 |
>   |:---:|---:|---|
>   | A | 5 | **ETL 失敗語意與缺口回補**（P0：F-030、F-043、F-045、F-050 ＋ 19 條 ETL 的 B 級）、[前端指標與報表同源化.md](../frontend/report-metrics.md)（**2026-09-09 完成，已移入 `docs/`**）（P1：F-082 ＋ F-083／F-084／F-085／F-067） |
...
>
> 全 repo 重掃一次後開了 `backlog/健檢殘留項目收斂.md`（P2，6 條）。**該文件已於 2026-09-04 六條全數收斂後依 `manage-backlog` §5 刪除**（commit `d69b9db`），結論寫在 [`code-quality.md`](code-quality.md) 的對應列；其後的第三輪見 [health-check-round3-2026-09.md](health-check-round3-2026-09.md)。
